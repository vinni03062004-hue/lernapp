...
export async function submitAttempt(input: SubmitAttemptInput): Promise<SubmitAttemptResult> {
  ...
  // KI-Bewertung nur bei Bedarf: Nur wenn die regelbasierte Rubrik die offene
  // Antwort NICHT perfekt per Stichwort abdeckt (falsch, teilweise ODER es
  // fehlt mind. ein Rubrik-Punkt), prüft Gemini semantisch gegen. So werden
  // korrekte, aber anders formulierte Antworten bzw. andere gültige Beispiele
  // fair bewertet; bei perfektem Stichwort-Treffer sparen wir den API-Aufruf.
  if (
    ['open', 'transfer', 'image_open'].includes(question.type) &&
    (!result.correct || result.rubricMisses.length > 0)
  ) {
